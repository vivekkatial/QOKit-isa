...
from skquant.interop.scipy import imfil, pybobyqa, nomad
from skquant.opt import minimize as skminimize
...
# Constants
CLASSICAL_OPTIMIZERS = ['IMFIL', 'PYBOBYQA', 'SLSQP', 'L-BFGS-B', 'COBYLA']
...
def run_qaoa(G, init_class, optimizer):
    """Run QAOA with qibpi initialization and compare classical optimizers."""
    layer =  N_LAYERS
    optimal_cut = get_optimal_cut(G)
    logging.info(f"Optimal cut: {optimal_cut}")
    init_class.current_layer = layer
    init_params = init_class.init_params_for_layer('qibpi', layer)
    gamma_init, beta_init = init_params[:layer], init_params[layer:]

    logging.info(f"Optimizing with {optimizer} optimizer.")
    ...
    bounds = [[-2*np.pi, 2*np.pi] for _ in range(2 * layer)]
    # Make a numpy array
    bounds = np.array(bounds)
    if optimizer in ['IMFIL', 'PYBOBYQA', 'NOMAD']:
        def fun_wrapper(fun, callback):
            def wrapped_fun(x, *args):
                callback(x)  # Call the callback with the current parameters
                return fun(x, *args)
            return wrapped_fun
        
        wrapped_fun = fun_wrapper(f, callback)

        if optimizer == 'IMFIL':
            res = minimize(wrapped_fun, theta, method=imfil, options={'budget' : 100}, bounds=bounds)
        elif optimizer == 'PYBOBYQA':
            bounds = np.array(bounds)
            res = minimize(wrapped_fun, theta, method=pybobyqa, options={'budget' : 100}, bounds=bounds, callback=callback)

    else:
        res = minimize(f, theta, method=optimizer, options={'maxfeval': 100}, bounds=bounds, callback=callback)


    print(f"Function value at the end of optimization: {res.fun}")
    # number of evals
    print(f"Number of evaluations: {res.nfev}")

    approximation_ratio = -f(res.x) / optimal_cut
    logging.info(f"Approximation ratio with {optimizer} optimizer: {approximation_ratio}")

    return approximation_ratio, evals

def main():
    logging.info("Starting QAOA with different classical optimizers")

    global GRAPH_TYPE, NUM_NODES, WEIGHT_TYPE, N_LAYERS, TRACK, CUSTOM_GRAPH
    ...
    if TRACK:
        mlflow.set_experiment("QAOA-Classical-Optimizers")
    ...
    # Run QAOA with qibpi initialization for each classical optimizer
    for optimizer in CLASSICAL_OPTIMIZERS:
        approximation_ratio, evals = run_qaoa(G, init_class, optimizer)
        results[optimizer] = approximation_ratio
        evaluations[optimizer] = evals
    ...
    df['new_method'] = df['method'].apply(lambda x: x.split('_')[0])
    ...
    for method in df['new_method'].unique():
        ...
        if num_evals is np.nan:
            # Allocate penalty value if the method did not reach the acceptable ratio, the penalty value is 100000
    ...
    ## Create plots
    with make_temp_directory(local=True) as temp_dir:
